...
use Octobro\API\Classes\Transformer\IncludesWithBrackets;
use Octobro\API\Classes\Transformer\VirtualUuid;
use Str;
...
{
    use ExtendableTrait, EloquentModelRelationFinder, DynamicInclude, IncludesWithBrackets;
    ...
    {
        return array_merge($this->availableIncludes, [self::VIRTUAL_FIELD_UUID]);
    }
    ...
    public function processIncludedResources(Scope $scope, $data)
    {
        ...
        if (!$scope->getScopeIdentifier()) {
            ...
            if ($includes) {
                ...
                if (Transformer::containsBracketNotation($includes)) {
                    $includes = Transformer::transformBracketToDotNotation($includes);
                }
